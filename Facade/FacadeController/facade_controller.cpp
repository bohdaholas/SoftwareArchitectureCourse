...
void FacadeController::Server::connect_to_other_microservices(io_service &io_service) {
    ...
    message_service_sockets.reserve(opt.message_microservices_count);
    ...
    for (size_t i = 0; i < opt.message_microservices_count; ++i) {
        tcp::endpoint message_service_endpoint(ip::address::from_string("127.0.0.1"), opt.message_microservice_port + i);
        message_service_sockets.emplace_back(std::make_shared<tcp::socket>(io_service));
        std::string str_id = "message microservice #"s + std::to_string(opt.message_microservice_port + i);
        async_connect_with_retry(*(message_service_sockets[i]), message_service_endpoint, RETRY_COUNT,
                                 str_id, FacadeController::Server::async_connect_cb);
    }
}
                          ...
                          [this](boost::system::error_code error) {
                              ...
                              if (!error) {
                                  auto session = std::make_shared<Session>(*this, std::move(client_socket),
                                                                           ...
                                                                           message_service_sockets[message_service_idx],
                                                                           opt);
                                  session->start();
                                  active_sessions.push_back(session);
                              }
                              logging_service_idx = (logging_service_idx + 1) % opt.logging_microservices_count;
                              message_service_idx = (message_service_idx + 1) % opt.message_microservices_count;
                              accept_client_connection();
                          });
}
